...
def manage_skills(request):
    ...
    if request.method == 'POST':
        ...
        elif 'update_certifications' in request.POST:
    ...
    # Get all skills and group them by normalized category
    all_skills = Skill.objects.all()
    
    # Prepare skills for template with proper grouping by category
    category_list = {}
    for skill in all_skills:
        ...
        category_list[category].append(skill)
    
    # Sort categories alphabetically
    sorted_categories = sorted(category_list.items())
    ...
    context = {
        ...
        'category_list': sorted_categories
    }
...
def job_list(request):
    ...
    # Format the dates for each job
    for job in jobs:
        if job.deadline:
            job.deadline_str = job.deadline.strftime("%b %d, %Y")
        else:
            job.deadline_str = "No deadline set"
...
def job_detail(request, job_id):
    ...
    # Find similar jobs based on department or title keywords (limit to 3)
    similar_jobs = JobPosting.objects.filter(
        is_active=True
    ).exclude(
        id=job_id
    ).filter(
        department__icontains=job.department
...
def create_job(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            job.save()
            # For many-to-many relationships, we need to save the form again
            form.save_m2m()
...
def staff_list(request):
    ...
    # Debug output
    print("Staff users:", [(user.username, user.role) for user in staff_users])
...
def edit_staff(request, user_id):
    """Edit staff member profile (Admin only)"""
    # Ensure user is an admin/superuser
    if not request.user.is_superuser:
        messages.error(request, "Access denied. Admin privileges required.")
        return redirect('home')
    
    # Get the staff user to edit
    staff_user = get_object_or_404(User, id=user_id)
    
    # Ensure the user is a staff member (HR Coordinator or Interviewer)
    if staff_user.role not in [User.HR_COORDINATOR, User.INTERVIEWER]:
    ...
    if request.method == 'POST':
        ...
        elif role not in [User.HR_COORDINATOR, User.INTERVIEWER]:
            messages.error(request, "Invalid role selected.")
        else:
            # Update user data
...
# Add these forms near your other form classes
...
# Add these functions to the views.py file
def is_admin_or_hr(user):
    return user.is_staff or user.is_superuser or user.role == 'hr_coordinator'

@login_required
@user_passes_test(is_admin_or_hr)
def admin_skills(request):
    """Admin page to manage all skills in the system"""
    skills = Skill.objects.all().order_by('category', 'name')
    
    # Group skills by category
    categories = {}
    for skill in skills:
        # Normalize category name (trim whitespace and capitalize consistently)
        category = skill.category.strip()
...
def admin_add_skill(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f"Skill '{form.cleaned_data['name']}' added successfully.")
    ...
    return render(request, 'admin_skill_form.html', context)
...
def admin_delete_skill(request, skill_id):
    ...
    return render(request, 'admin_skill_confirm_delete.html', context)

@login_required
@user_passes_test(is_admin_or_hr)
def normalize_skill_categories(request):
    """Fix duplicate categories in the skills database by normalizing them"""
    if not request.user.is_superuser:
    ...
    # Create a dictionary to track normalized categories
    normalized_categories = {}
    changes_made = 0
    
    # First pass: identify all unique normalized categories
    for skill in skills:
        normalized_category = skill.category.strip()
        if normalized_category not in normalized_categories:
            normalized_categories[normalized_category] = []
        normalized_categories[normalized_category].append(skill)
    
    # Second pass: update all skills to use the normalized category
    for normalized_category, skill_list in normalized_categories.items():
    ...
    if changes_made > 0:
        messages.success(request, f"Category normalization complete! Fixed {changes_made} skills with inconsistent categories.")
    else:
        messages.info(request, "All categories are already normalized. No changes were needed.")
